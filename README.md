...
此版本不使用 LINE。顧客掃桌上的 QR Code，進入網站後以「一問一答」方式完成選系列 → 選飲料 → 甜度 → 冰熱 → 杯數 → 訂單確認；還能向 AI 詢問推薦。店員從管理頁查看與更新訂單。
...
掃 A1 QR → 選「厚奶茶」→「覺醒奶茶」→「3分糖」→「完全去冰」→「2 杯」→「確認送單」→ 店員於 /admin 輸入權杖，查看 NT$110 的訂單並改為已接單。也可在下方輸入「推薦不含奶的飲料」體驗 AI 問答。
